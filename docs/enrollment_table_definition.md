...
| `TBL_ENROLLMENT` | 수강신청 정보 | 학생, 과정, 강사, 시작일, 횟수, 시작시간, 수업시간 저장 |
...
| `lessonCount` | `SMALLINT` | N |  | 수업횟수. 예: 32회, 40회 |
...
| `IX_TBL_ENROLLMENT_TUTOR_TIME` | `tutorId`, `lessonStartDate`, `lessonStartTime` | 강사별 수업 시간 조회 |
...
- 요일은 `월수금` 같은 문자열 하나로 저장하지 않고 `TBL_ENROLLMENT_WEEKDAY`에 숫자 여러 건으로 저장했다. 이렇게 하면 월수금, 화목, 월수, 화목금 같은 조합을 모두 처리할 수 있다.
...
| 수강신청 | 5건 | 32회/40회, 20분/40분, 다양한 요일 패턴 |
| 수강신청 요일 | 12건 | 월수금, 화목, 월수, 화목금, 수금 |
| 일자별 수업정보 | 176건 | 수강신청의 수업횟수 합계만큼 자동 생성 |
